...
def publishContent():
        # acquire list of drafts via manager
        draftsList = manager.Manager(os.path.join(siteRoot, "_drafts")).getContents()
        if len(draftsList) > 0:
                info("These are your drafts:")
                
                for index, item in enumerate(draftsList):
                        print("\t", index, ":", item[0])
                print("\t q : quit")
                
                print("What would you like to publish? ", end="")
                ...
                if re.search("[^0-9 ]", rawInput) == None:
                        ...
                        for draft in selectedDrafts:
                                ...
                                if index < len(draftsList):
                                        publishDraft(draftsList[index])
                                else:
                                        error(draft + " does not correspond to a draft", exit=False)
                
                elif rawInput == "q":
                        info("No drafts published")
                else:
                        error("please input a number relative to a draft")
        else:
                info("You have no drafts to publish")

# Publish specified draft in the form returned by the manager
def publishDraft(draft):
        oldName = draft[0]
        oldPath = os.path.join(draft[1], oldName)
